package javasound.sampled;

import io.github.jseproject.ExtendedAudioFileFormat;
...
public final class Audios {
    ...
    private Audios() {
        throw new AssertionError("No sampled.javasound.Audios instances for you!");
    }
    ...
    public static AudioFileFormat.Type getFileTypeBySuffix(String suffix) {
        AudioFileFormat.Type type = null;
        for (FormatEncodingProvider provider : getFormatEncodingProviders()) {
            type = provider.getFileTypeBySuffix(suffix);
            if (type != null) break;
        }
        return type;
    }
     ...
     * @see #isWriterSupportedFileType
     * @see #getWriterFileTypes
     * @see AudioFormat#properties()
     */
    public static int write(AudioInputStream stream, AudioFileFormat.Type fileType, Map<String, Object> properties, OutputStream out)
            throws IOException, IllegalArgumentException {
        ...
        for (AudioCompressionWriter writer : getAudioCompressionWriters()) {
            try {
                return writer.write(stream, fileType, properties, out);
            }
     ...
     * @see AudioFormat#properties()
     */
    public static int write(AudioInputStream stream, AudioFileFormat.Type fileType, Map<String, Object> properties, File out)
            throws IOException, IllegalArgumentException {
        Objects.requireNonNull(stream);
        Objects.requireNonNull(fileType);
        Objects.requireNonNull(out);
